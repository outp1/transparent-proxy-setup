...
sudo iptables -A OUTPUT -p udp -m set --match-set openai dst --dport 443 -j REJECT --reject-with icmp-port-unreachable
...
sudo iptables  -t nat -A OUTPUT -p tcp -m set --match-set openai dst -m tcp --dport 80  -j REDIRECT --to-ports "$REDSOCKS_PORT"
...
sudo iptables  -t nat -A OUTPUT -p tcp -m set --match-set openai dst -m tcp --dport 443 -j REDIRECT --to-ports "$REDSOCKS_PORT"
...
if command -v ip6tables >/dev/null 2>&1; then
  ...
  sudo ip6tables -A OUTPUT -p udp -m set --match-set openai6 dst --dport 443 -j REJECT --reject-with icmp6-port-unreachable
  ...
  sudo ip6tables -t nat -A OUTPUT -p tcp -m set --match-set openai6 dst -m tcp --dport 80  -j REDIRECT --to-ports "$REDSOCKS_PORT"
  ...
  sudo ip6tables -t nat -A OUTPUT -p tcp -m set --match-set openai6 dst -m tcp --dport 443 -j REDIRECT --to-ports "$REDSOCKS_PORT"
fi
